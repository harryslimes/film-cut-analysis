"""Hybrid decision sheet: SSIM drops clean flashes, then density-collapse mops up the
dense bursts SSIM leaves behind (graveyard/green-Scotty/locket where flash+motion fools
SSIM). green=keep, red=dropped-by-ssim(flash), orange=dropped-by-density(residual burst)."""
...
a0 = a.start-1.0
def dec(vf, sc):
    return subprocess.run(["ffmpeg","-hide_banner","-loglevel","error","-ss",str(a0),"-t",
        str(a.end-a0+1),"-i",vf,"-vf",sc,"-f","rawvideo","-"],stdout=subprocess.PIPE,stderr=subprocess.PIPE).stdout
gray = np.frombuffer(dec(a.video,"scale=256:144,format=gray"),np.uint8).reshape([-1,144,256])
...
col = np.frombuffer(dec(a.video,f"scale={a.tw}:{th},format=bgr24"),np.uint8).reshape([-1,th,a.tw,3])
...
S={}
for c in cuts:
    ig=int(round((c-a0)*a.fps)); s=ssim(grayc[max(0,ig-d)],grayc[min(len(grayc)-1,ig+d)]); S[c]=s
    if s<a.thresh: ssim_keep.append(c)
# stage 2: density-collapse the survivors
final=set(dampen_strobe(ssim_keep, dens_window=8, dens_max=3, merge_gap=4, keep_gap=5.0))
...
print(f"{len(cuts)} cuts -> KEEP {nk}, flash-drop {nf}, burst-drop {nb} -> {a.out}")
